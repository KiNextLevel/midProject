...
<body class="ecommerce">
    ...
    <div class="header">
      <div class="container">
        <a class="site-logo" href="controller.jsp?command=MAINPAGE"><img src="assets/corporate/img/logos/logo-shop-red.png" alt="로고 이미지지"></a>
        ...
        <div class="row margin-bottom-40">
          ...
          <div class="col-md-9 col-sm-9">
           ...
           <%//BoardDTO boardDTO = (BoardDTO)request.getAttribute("boardDTO");%>
            <h1>여기 이벤트 제목${boardDTO.getBoard_Title}</h1>
            <div class="content-page">
              <p>이벤트 내용</p>
              <p>${boardDTO.getBOARD_CONTENT}</p>
              <p>이벤트 날짜</p>
              <p>${boardDTO.getBOARD_DATE}</p>
              <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                <%if(session.getAttribute("userRole").equals("USER")){%>
                  <form action="controller.jsp" method="post">
                    <input type="hidden" name="command" value="PARTICIPANT">
                     <input type="hidden" name="boardNum" value="${boardDTO.Board_Num}">
                  <input type="submit" value="참가하기">
                 </form>
                <%}else if(session.getAttribute("userRole").equals("ADMIN")){%>
                  <table border="1">
                    <tr>
                      <td><a href="controller.jsp?action=MODIFYBOARDPAGE&boardNum=<%//=boardDTO.Board_Num()%>">수정</a></td>
                      <td><a href="controller.jsp?action=DELETEBOARD&boardNum=<%//=boardDTO.Board_Num()%>">삭제</a></td>
                    </tr>
                  </table>
                  <%}%>
                <span style="font-size: 14px; color: #555;">현재 참가 인원 ${board_count} / ${boardDTO.getBoard_Limit}</span>
            </div>
            </div>
          </div>
          <!-- END CONTENT -->
